	...
	case string:
		for _, c := range a {
			if _, err := strconv.Atoi(string(c)); err == nil {
				log.Fatal("ERROR - CREATE COMMENT - string supplied for tag but integer found in tag. tag consist of non digits")
			}
		}
...
func getContent(db *sql.DB, eventID int, arg interface{}) (response interface{}, retErr error) {
	...
	case string:
		for _, c := range a {
			if _, err := strconv.Atoi(string(c)); err == nil {
				log.Fatal("ERROR - GET CONTENT - string supplied for tag but integer found in tag. tag consist of non digits")
			}
		}
